	...
	const fetchCampaignDetail = async () => {
		await axios
			.request({
				...
				url: `${api.BASE_URL}/web/campaign/project/${id}`,
				headers: {
			...
			.then(async (response) => {
				...
				if (!response.data?.qrCode) {
					await axios.request({
						method: "get",
						maxBodyLength: Infinity,
						url: `${api.BASE_URL}/web/campaign/generate-qr-code/${id}?url=${api.WEBSITE}/projects/${id}`,
						headers: {
							"Content-Type": "application/json",
						}
					}).then((res) => {
			...
			.catch((e) => {
				console.error(e);
				router.replace(`/projects/${router.query.id}`);
			});
	};
